--------------------------------------------------------------------------------
-- |
-- Module      : XMonad.Prompt.Fuzzy
-- Copyright   : (C) 2015 Norbert Zeh, 2018 Evgeny Kurnevsky
-- License     : AGPL
--
...
module XMonad.Prompt.Fuzzy where
...
fuzzyCompl _ [] = []
fuzzyCompl l q = fuzzySort q $ filter (fuzzyMatch q) $ filter (\e -> length e >= length q) l
...
fuzzySort q = map snd . sort . map ranked
  where ranked s = (fromMaybe (maxBound, maxBound) $ rankMatch q s, s)

rankMatch :: String -> String -> Maybe (Int, Int)
rankMatch q s = if null matches
                then Nothing
                else Just $ minimum matches
...
findShortestMatches q  s =
  case map (findOccurrences s) q of
    (os:oss) -> foldl' extendMatches spans oss where spans = map (\o -> (o, o)) os
    _        -> []
